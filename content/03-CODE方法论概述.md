...
但如果你要用 AI 做一个**完整的功能或项目**呢？比如从零开发一个 Todo API——有需求分析、架构设计、编码、测试、审查……这时候光会"写提示词"不够了，你需要一套**流程**来组织这些工作。
...
**举个例子**：确定用 FastAPI 框架、PostgreSQL 数据库、JWT 认证、Docker 部署，设计好目录结构和 API 接口规范。
...
**CODE 的完整流程适用于中大型功能开发**——比如从零搭建一个服务、开发一个完整模块。
...
**不要把 CODE 当成每次都必须走的仪式，把它当成复杂任务的导航地图。** 路线简单的时候凭直觉走就行，路线复杂的时候才需要看地图。
...
- O 阶段：角色扮演（让 AI 扮演架构师审查设计）
- D 阶段：挑错（让 AI 找出代码 bug）
- E 阶段：角色扮演（让 AI 扮演 QA/安全专家评估）

**三者的关系：CRC 是砖头，多轮对话是砌墙的手法，CODE 是房子的图纸。**
